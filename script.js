...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initTheme(); initMiniMap(); initDiagramInteractions();
...
function handleInput(e) {
    if (e.key === 'ArrowRight' || e.key === ' ' || e.key === 'Enter') {
        e.preventDefault();
        if (state.currentSlide < state.slides.length - 1) scrollToSlide(state.currentSlide + 1);
...
function setTheme(theme) {
    ...
    document.querySelectorAll('.theme-btn').forEach(btn => {
        ...
        if (btn.getAttribute('onclick').includes(theme)) btn.classList.add('active');
    });
}
...
function initDiagramInteractions() {
    ...
    document.querySelectorAll('.d-box').forEach(box => {
        box.addEventListener('mouseenter', () => {
            ...
            if (title) {
                titleEl.innerText = title; descEl.innerText = box.getAttribute('data-desc');
                popup.classList.add('active');
                popup.className = 'info-popup active';
                if (box.classList.contains('d-sensor')) { popup.classList.add('left'); popup.style.borderLeftColor = box.style.borderLeftColor || 'var(--accent)'; }
                else if (box.classList.contains('d-output')) { popup.classList.add('right'); popup.style.borderLeftColor = 'var(--alert)'; }
                else { popup.classList.add('center'); popup.style.borderLeftColor = '#1e40af'; }
            }
        });
...
document.getElementById("connectBtn").addEventListener("click", async () => {
    if ("serial" in navigator) {
        ...
        } catch (err) { sim("Error: " + err, "alert"); }
    } else { alert("Use Chrome/Edge/Brave"); }
});